...
const titleComponents: JSXMapSerializer = {
  heading1: ({ children }) => (
    <h1 className="mt-4 font-sans text-4xl font-semibold leading-[1.05] tracking-tight text-neutral-900 sm:text-[42px] lg:text-5xl">
      {children}
    </h1>
  ),
  em: ({ children }) => (
    <em className="not-italic text-brand-500">{children}</em>
  ),
};

const descriptionComponents: JSXMapSerializer = {
  paragraph: ({ children }) => (
    <p className="mt-5 max-w-[52ch] text-[17px] leading-[1.6] text-neutral-600">
      {children}
    </p>
  ),
};
...
const statsComponents: JSXMapSerializer = {
  paragraph: ({ children }) => (
    <p className="mt-6 flex items-center gap-2 text-[13.5px] text-neutral-500">
      <span
  ...
  strong: ({ children }) => (
    <b className="font-semibold text-neutral-900">{children}</b>
  ),
};
  ...
  return (
    <section className="mx-auto grid max-w-[1640px] items-center gap-10 px-4 pt-12 pb-12 sm:px-6 lg:grid-cols-[1.05fr_1fr] lg:gap-14 lg:px-7 lg:pt-16 lg:pb-14">
      ...
      <div className="relative mx-auto w-full max-w-[520px] lg:max-w-none">
        <div className="relative aspect-[4/3.4] overflow-hidden rounded-[28px] bg-[linear-gradient(150deg,var(--decor-hero-from),var(--decor-hero-to))] shadow-card">
          <PrismicImage
            field={image}
            fill
            loading="eager"
        ...
        {metrics.length > 0 && (
          <div className="absolute -right-4 -bottom-4 flex gap-5 rounded-[20px] bg-white p-5 shadow-card sm:-right-5 sm:-bottom-5">
            {metrics.map((metric, index) => (
              <div key={metric.label ?? index}>
                <b className="block font-sans text-[26px] font-semibold leading-none text-neutral-900">
                  {metric.value}
                </b>

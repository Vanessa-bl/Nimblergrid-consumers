  ...
  return (
    <>
      <button
        ...
        onClick={onPrevious}
        className={`absolute left-2 top-1/2 z-10 flex h-9 w-9 cursor-pointer -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-neutral-700 shadow-md backdrop-blur-sm transition-colors hover:bg-white hover:text-neutral-900 ${focusVisibleClasses}`}
      >
      ...
      <button
        ...
        onClick={onNext}
        className={`absolute right-2 top-1/2 z-10 flex h-9 w-9 cursor-pointer -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-neutral-700 shadow-md backdrop-blur-sm transition-colors hover:bg-white hover:text-neutral-900 ${focusVisibleClasses}`}
      >
      ...
      {images.map((image, index) => {
        ...
        return (
          <button
            ...
            className="group/dot pointer-events-auto flex h-5 w-5 cursor-pointer items-center justify-center rounded-full focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-brand-500"
          >
            <span
              aria-hidden="true"
...
function BadgeTag({ badge }: Readonly<{ badge: string }>) {
  ...
  return (
    <span className="absolute left-3 top-3 rounded-full bg-white/95 px-2.5 py-1 text-[11px] font-semibold text-neutral-900 shadow-sm backdrop-blur-sm">
      {badge}
    </span>
  );
}
...
function GalleryMedia({
  ...
  const mediaClassName = stacked
    ? "relative aspect-[16/10] shrink-0 touch-pan-y overflow-hidden bg-neutral-100"
    : "relative aspect-[16/10] shrink-0 touch-pan-y overflow-hidden bg-neutral-100 md:aspect-auto md:h-full md:min-h-[280px]";
...
function PropertyHeader({
  ...
  return (
    <>
      {propertyType !== "" ? (
        <span className="text-[11px] font-bold uppercase tracking-[0.12em] text-brand-500">
          {propertyType}
        </span>
      ) : null}
      {title !== "" ? (
        <h3
          title={title}
          className="mt-1.5 line-clamp-2 font-sans text-[20px] font-semibold leading-tight tracking-tight text-neutral-900"
        >
          {title}
        </h3>
      ) : null}
      <p className="mt-2 text-[24px] font-bold leading-tight text-neutral-900 md:text-[28px]">
        {formatPrice(price)}
      </p>
    </>
  );
...
function PropertyFeatures({
  ...
  return (
    <ul className="mt-4 grid grid-cols-2 gap-y-2 md:flex md:items-center md:divide-x md:divide-neutral-200">
      {items.map((feature) => (
        <li
          key={feature.id}
          className="text-[13px] text-neutral-600 md:px-3 md:first:pl-0 md:last:pr-0"
        >
          {feature.label}
        </li>
      ))}
...
function PropertyFooter({
  ...
  return (
    <footer className="mt-4 flex flex-col gap-3.5 border-t border-neutral-100 pt-4 md:flex-row md:items-center md:justify-between">
      {showAgent ? (
        <div className="flex min-w-0 items-center gap-3">
          {agentLogo !== "" ? (
            ...
            <span
              aria-hidden="true"
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-neutral-200 text-[13px] font-semibold text-neutral-600"
            >
      ...
      <div className="flex flex-wrap items-center gap-2">
        {phone !== "" ? (
          <a
            ...
            className={`inline-flex h-11 w-11 cursor-pointer items-center justify-center rounded-xl border border-neutral-200 text-neutral-700 transition-colors hover:bg-neutral-50 ${focusVisibleClasses}`}
          >
        ...
        {whatsappHref !== null ? (
          <a
            ...
            className={`inline-flex h-11 cursor-pointer items-center justify-center gap-2 rounded-full border border-neutral-200 px-4 text-sm font-semibold text-neutral-700 transition-colors hover:bg-neutral-50 ${focusVisibleClasses}`}
          >
...
export const PropertyCard = memo(function PropertyCard(
  ...
  return (
    <article
      className={`group flex flex-col overflow-hidden rounded-[20px] border bg-white shadow-subtle transition-shadow duration-300 hover:shadow-card-hover ${layoutClass} ${
        isSelected ? "border-brand-500 ring-2 ring-brand-500/20" : "border-neutral-200"
      }`}
    >
      <GalleryMedia
      ...
      {contactStatus !== undefined ? (
        <div className="flex items-center justify-center gap-2 bg-neutral-900 px-4 py-2.5 md:col-span-2">

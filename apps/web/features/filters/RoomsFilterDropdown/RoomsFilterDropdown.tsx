...
function SegmentedGroup(props: Readonly<SegmentedGroupProps>) {
  ...
  return (
    <div role="group" aria-label={groupLabel} className="flex overflow-hidden rounded-lg border border-neutral-200 bg-white">
      {options.map((option, index) => {
        ...
        return (
          <button
            ...
            className={`flex-1 cursor-pointer whitespace-nowrap border-r border-neutral-200 px-1 py-2 text-center text-[13px] font-medium transition-colors focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-brand-500 ${
              isLast ? "border-r-0" : ""
            } ${isSelected ? "bg-neutral-900 text-white" : "bg-white text-neutral-700 hover:bg-neutral-100"}`}
          >
            {option}
          </button>
        );
      ...
      {open ? (
        ...
        >
          <div className="flex items-center justify-between">
            <h2 className="text-base font-semibold text-neutral-900">Rooms</h2>
            <button
              type="button"
              onClick={handleDone}

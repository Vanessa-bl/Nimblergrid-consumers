    ...
    <div className="mt-4 flex h-16 items-end gap-[3px]" aria-hidden="true">
      {distribution.map((value, index) => {
        ...
        return (
          <div
            key={index}
            className={`flex-1 rounded-t-sm ${inRange ? "bg-rose-500/80" : "bg-zinc-200"}`}
            style={{ height: `${height}%` }}
          />
        );
      })}
...
function SliderTrack(props: Readonly<SliderTrackProps>) {
  ...
  return (
    <div className="relative mt-2 h-6">
      <div className="absolute left-1 right-1 top-1/2 h-1 -translate-y-1/2 rounded-full bg-zinc-200" />
      <div
    ...
    <label className="block">
      ...
      <span className="relative mt-1 block">
        <select
          ...
          className="h-11 w-full cursor-pointer appearance-none rounded-xl border border-zinc-300 bg-white px-3 pr-8 text-sm text-zinc-900 outline-none transition-colors hover:border-zinc-400 focus:border-zinc-900"
        >
    ...
    <div ref={wrapperRef} className="relative">
      ...
      {open ? (
        ...
        >
          ...
          <div className="mt-1 flex items-center justify-between text-sm font-medium text-zinc-900">

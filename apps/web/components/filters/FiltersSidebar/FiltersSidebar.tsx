...
import {
  ...
  formatPriceOption,
  getStepOptions,
} from "@/components/filters/PriceRangeDropdown/PriceRangeDropdown";
import {
  BATHROOM_OPTIONS,
  BEDROOM_OPTIONS,
  SegmentedGroup,
  numericOptionsUpTo,
} from "@/components/filters/RoomsFilterDropdown/RoomsFilterDropdown";
import { PropertyTypeGrid } from "@/components/filters/PropertyTypeDropdown/PropertyTypeDropdown";
import {
  EMPTY_FILTERS_STATE,
    ...
    <section>
      <h3>
        <button
          ...
          className="flex w-full cursor-pointer items-center justify-between py-4 text-left text-[15px] font-semibold text-zinc-900 transition-colors hover:text-rose-600 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-rose-500"
        >
          {title}
          <ChevronDownIcon
    ...
    <>
      ...
      <Accordion title="Rooms" defaultOpen={true}>
        <div className="space-y-5">
          <div>
            <h4 className="mb-2 text-sm font-semibold text-zinc-900">Bedrooms</h4>
            <SegmentedGroup
              groupLabel="Bedrooms"
          ...
          <div>
            <h4 className="mb-2 text-sm font-semibold text-zinc-900">Bathrooms</h4>
            <SegmentedGroup
      ...
      <Accordion title="Expanded search" defaultOpen={false}>
        <label className="block">
          ...
          <input
            ...
            className="h-11 w-full rounded-xl border border-zinc-300 bg-white px-3 text-sm text-zinc-900 outline-none transition-colors hover:border-zinc-400 focus:border-zinc-900"
          />
        </label>
      </Accordion>
    </>
      ...
      >
        <header className="flex shrink-0 items-center justify-between border-b border-zinc-200 px-6 py-4">
          ...
          <button
            ...
            onClick={onClose}
            className="grid h-9 w-9 cursor-pointer place-items-center rounded-full text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-900 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-rose-500"
          >
            <CloseIcon className="h-5 w-5" />
          </button>
        </header>

        <div className="flex-1 divide-y divide-zinc-200 overflow-y-auto px-6">
          <SidebarSections filters={filters} patch={patch} />
        </div>

        <footer className="flex shrink-0 items-center justify-between border-t border-zinc-200 bg-white p-4 px-6">
          <Button
            type="button"

...
const baseButtonClasses =
  "inline-flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border text-sm font-semibold transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-500";

const idleButtonClasses =
  "border-neutral-200 bg-white text-neutral-700 hover:border-neutral-400 hover:text-neutral-900";

const activeButtonClasses = "border-transparent bg-neutral-900 text-white";

const disabledButtonClasses =
  "pointer-events-none cursor-not-allowed border-neutral-200 bg-white text-neutral-300";
        ...
        {tokens.map((token) =>
          token.kind === "gap" ? (
            <li
              key={`gap-${token.after}`}
              aria-hidden="true"
              className="flex h-10 w-6 items-center justify-center text-sm text-neutral-400"
            >
              …
            </li>
          ) : (
